...
def request_api(  # pylint: disable=too-many-positional-arguments
    ...
    if resp.status_code == 200:
        ...
        detail = resp_json["detail"] if resp_json and "detail" in resp_json else resp.text
        raise click.ClickException(
            "Registry internal server error: {}".format(detail)
        )
        ...
        detail = resp_json["detail"] if resp_json and "detail" in resp_json else resp.text
        raise click.ClickException(
            "Conflict in Registry. {}".format(detail)
        )
