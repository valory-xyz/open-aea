...
9. **libp2p v0.33 default ResourceManager — stream-per-peer cap breaks ACN routing under burst load** ✓ fixed.

   **Symptom**: `TestLibp2pEnvelopeOrder::test_burst_order` (all 3 variants) and `TestMessageOrderingWithDelegateClientTwoHops` fail after ~82 envelopes. The Go test shows errors like `Expected counter 87 received counter 85`; the Python burst test times out on receive with zero envelopes delivered on the Python side.

   **Root cause**: libp2p v0.33 installs a default ResourceManager that caps per-peer streams at 256 in / 256 out. The legacy v0.8 host had no resource manager at all. Our ACN routing pattern opens a fresh `/aea-address` stream pair for every envelope against the same destination peer, so a 1000-envelope burst between two peers cliffs at the cap. Subsequent `NewStream` calls fail with `stream reset`, which then cascades through a latent variable-shadowing bug in `lookupAddressDHT` (the inner `record, err := ...` shadows the outer `err`, so on the read failure the outer `err` stays nil from the earlier successful `NewStream`). The function returns `peerID="" err=nil` to `_routeEnvelopeDHTLookup`, which skips the error check and passes the empty peer ID to `routedHost.NewStream` → `empty peer ID` error. Those envelopes get pushed to the slow_queue and stay broken.

   **Fix**: pass `libp2p.ResourceManager(&network.NullResourceManager{})` to `libp2p.New` in `dht/dhtpeer/dhtpeer.go`. Restores pre-bump behaviour. Verified locally: `test_burst_order` (all 3 Python variants) + `TestMessageOrderingWithDelegateClientTwoHops` (Go, 5/5 runs) now pass. The `t.Skip` on the Go test has been removed.

   **Still-latent follow-ups** (not blocking, not triggered in tests after the fix, but still real bugs in the code):

   a. **ACN per-pair ordering is only preserved on the fast path.** The open-acn README promises "ACN should guarantee total ordering of messages for all agent pairs". The per-pair serializer `syncMessages[pair]` wraps calls to `RouteEnvelope(e)` in one goroutine per `(sender, recipient)` pair, which looks like it enforces that. But inside `RouteEnvelope`, when a destination address lookup misses the local DHT cache, the envelope is pushed to the **global** `slow_queue` (a single shared `chan *aea.Envelope` drained by exactly one `slowEnvelopeSendLoop` goroutine) and `RouteEnvelope` returns immediately. The per-pair goroutine then processes envelope N+1, which may hit the cache (now populated by N's lookup side effects) and take the fast direct-delivery path — so envelope N+1 is delivered before envelope N. Per-pair ordering is silently violated. With the resource-manager fix, the tests no longer hit this path because the stream-reset cascade that was forcing envelopes into slow_queue is gone. But any GENUINE DHT miss — cold start, real network flake, large topology where the destination isn't in the local routing table yet — still triggers it, and the ordering guarantee still doesn't hold. Fix options: (B) run the DHT lookup retry loop inline in the per-pair goroutine so envelope N+1 blocks until N has either succeeded or exhausted retries; (C) make the slow queue per-pair instead of a global shared channel. Option B is simpler but changes throughput under genuine degradation; option C preserves the throughput profile but costs one goroutine per active pair. Recommend option B as the default next step.

   b. **Variable shadowing in `lookupAddressDHT` (`dhtpeer.go:1407`).** The inner `record, err := acn.PerformAddressLookup(streamPipe, address)` uses `:=` which creates a new `err` scoped to the inner loop iteration. The `if err != nil { continue }` checks the inner one. When the loop exits (all providers tried, all failed), the outer `err` is still whatever the last successful outer assignment was — in the failure case it's the nil from the earlier `stream, err = NewStream(...)` success. Return path at line 1461 (`return "", nil, err`) then returns `peerID="", err=nil` to the caller, which skips the error check. Minimal fix: rename the inner `err` to `lookupErr` or assign it back to the outer `err` before `continue`. This was the cascade amplifier under the resource-manager failure and it's still worth fixing defensively.
