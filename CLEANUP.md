...
9. **libp2p v0.33 default ResourceManager — stream-per-peer cap breaks ACN routing under burst load** ✓ fixed.
   ...
   **Still-latent follow-ups:**

   a. **ACN per-pair ordering is only preserved on the fast path** (still open). The open-acn README promises "ACN should guarantee total ordering of messages for all agent pairs". The per-pair serializer `syncMessages[pair]` wraps calls to `RouteEnvelope(e)` in one goroutine per `(sender, recipient)` pair, which looks like it enforces that. But inside `RouteEnvelope`, when a destination address lookup misses the local DHT cache, the envelope is pushed to the **global** `slow_queue` (a single shared `chan *aea.Envelope` drained by exactly one `slowEnvelopeSendLoop` goroutine) and `RouteEnvelope` returns immediately. The per-pair goroutine then processes envelope N+1, which may hit the cache (now populated by N's lookup side effects) and take the fast direct-delivery path — so envelope N+1 is delivered before envelope N. Per-pair ordering is silently violated. With the resource-manager fix, the tests no longer hit this path because the stream-reset cascade that was forcing envelopes into slow_queue is gone. But any GENUINE DHT miss — cold start, real network flake, large topology where the destination isn't in the local routing table yet — still triggers it, and the ordering guarantee still doesn't hold. Fix options: (B) run the DHT lookup retry loop inline in the per-pair goroutine so envelope N+1 blocks until N has either succeeded or exhausted retries; (C) make the slow queue per-pair instead of a global shared channel. Option B is simpler but changes throughput under genuine degradation; option C preserves the throughput profile but costs one goroutine per active pair. Recommend option B as the default next step. Tracked as a standalone follow-up PR.

   b. ~~**Variable shadowing in `lookupAddressDHT`**~~ ✓ fixed in commit `a3c9a74d2`. The inner `record, err :=` (and the two subsequent `valid, err :=` / `peerid, err :=`) used `:=` which created a new `err` shadowing the outer one. On a stream-reset mid-read the outer `err` stayed nil from the earlier successful `NewStream`, and the loop-exit return at line 1461 returned `peerID="" err=nil` to the caller — which then passed the empty peer ID to `routedHost.NewStream` and got the misleading "empty peer ID" error. Now all three assignments use `=` against an outer-scope declaration so failures propagate; inline comment at the site references this section.
