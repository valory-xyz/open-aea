...
- `open-aea-ledger-ethereum`: adds chain-aware fallback defaults inside the plugin itself. `get_default_gas_strategy(chain_id)` now returns per-chain `fallback_estimate` values for Optimism / Base / Mode / Fraxtal (5 gwei maxFeePerGas, 3 gwei tip — OP-stack), Arbitrum One (2 gwei maxFeePerGas, 1 gwei tip — above the 0.1 gwei floor), Polygon (6000 gwei maxFeePerGas, 30 gwei tip — clears the validator-enforced 25 gwei minimum priority fee, `max_gas_fast` 10000), and Celo (30 gwei maxFeePerGas — above the 25 gwei network minimum); all other chains inherit the existing `DEFAULT_FALLBACK_ESTIMATE`. Applies whether the ledger is built via `make_ledger_api('ethereum', chain_id=...)` directly OR through the `valory/ledger` connection, and covers both `eip1559` and `eip1559_polygon` strategies. User-supplied `gas_price_strategies` kwargs still take precedence. #775
...
- `valory/ledger` connection: aligns per-chain `fallback_estimate` in `connection.yaml` with the new plugin-level defaults — Optimism / Base / Mode / Fraxtal 5 gwei, Arbitrum 2 gwei, Polygon 6000 gwei + 30 gwei tip + `max_gas_fast` 10000, Celo 30 gwei. Technically redundant now that the plugin ships matching defaults, but keeps the YAML self-documenting and defensible if the plugin defaults ever drift. #775
