...
The HTTP client connection receives request envelops from an agent blueprint's skill, translates each into an HTTP request and sends it to a server external to the agent. If it receives an HTTP response from the server within a timeout window, it translates it into a response envelope, and sends this back to the relevant skill inside the agent.
...
aea create my_aea
cd my_aea
aea add connection valory/http_server:0.22.0:bafybeicha7kwh3nybevwgphnpbikv4qjyuhnbr6xzgkj7awgfhruehb3fq --remote
aea push connection valory/http_server --local
aea add protocol fetchai/default:1.0.0:bafybeifipcrdiu27t5vg3zgvqc4dpcspskhtq4iqyugg5ktumf7ca7w46q --remote
aea push protocol fetchai/default --local
aea add protocol valory/http:1.0.0:bafybeiaqxv3hlfnwf5tvbspxlwtqcrpkpxzaeymvoobvjws3z5htoayb6u --remote
