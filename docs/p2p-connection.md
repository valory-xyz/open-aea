The `valory/p2p_libp2p:0.1.0` connection allows AEAs to create a peer-to-peer communication network. In particular, the connection creates an overlay network which maps agent instances' public keys to IP addresses.

## Local demo

<div class="admonition note">
  <p class="admonition-title">Note</p>
  <p>The <code>valory/p2p_libp2p</code> connection uses two keys: a
  <code>cosmos</code> key for the libp2p node identity (the Go
  node handles secp256k1/cosmos derivations natively) and a second
  key (by default <code>ethereum</code>) for signing the
  <a href="../por">proof-of-representation</a> certificate. Both
  are set up in the commands below.
</p>
</div>
...
cd my_genesis_aea
aea add connection valory/p2p_libp2p:0.1.0:bafybeic7ksagzxqwzwnkxw2twwzr7qprpcolet2vesbr4kfbcpxzmurpsq --remote
