...
2. Determine the next AEA version: Create new release branch named "release/{new-version}, switch to this branch and run `aea-dev bump-version --new-version NEW_VERSION_HERE` (provided by the `open-aea-dev-helpers` plugin). Commit if satisfied.

3. Bump plugin versions if necessary by running `aea-dev update-plugin-versions --update "PLUGIN_NAME,NEW_VERSION"`. Commit if satisfied.
