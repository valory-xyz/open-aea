...
import pytest
from aea_ledger_ethereum.chainlist import (
    ChainlistRPC,
...
class TestProbeRpc:
    ...
    @patch("aea_ledger_ethereum.chainlist.urllib.request.urlopen")
    def test_null_result_returns_none(self, mock_urlopen: MagicMock) -> None:
...
class TestRPCNode:
    ...
    def test_tracking_yes(self) -> None:
        """Tracking 'yes' is flagged."""
        node = RPCNode(url="https://x.com", is_working=True, tracking="yes")
        assert node.is_tracking is True
...
class TestRankAndSelect:
    ...
    def test_filters_stale(self) -> None:
        ...
        results = [
            ...
            ("https://stale.com", 30.0, 900),  # 100 blocks behind
...
class TestChainlistRPC:
    ...
    @patch("aea_ledger_ethereum.chainlist.probe_rpc")
    def test_get_validated_rpcs_full_pipeline(
        ...
        mock_get_rpcs.return_value = [
            RPCNode(url="https://rpc.com/${API_KEY}", is_working=True),  # filtered
    ...
    def test_returns_empty_when_all_probes_fail(
        ...
        """Returns empty when all probes fail."""
        mock_get_rpcs.return_value = [
            RPCNode(url="https://dead.com", is_working=True),
        ]
...
# ---------------------------------------------------------------------------
# Fetch data and caching
# ---------------------------------------------------------------------------


class TestFetchData:
    """Tests for fetch_data caching behavior."""

    def test_uses_cache_when_valid(self, tmp_path: "pytest.TempPathFactory") -> None:
        """Valid cache is used without network fetch."""
        cache_file = tmp_path / "rpcs.json"
        cache_data = [{"chainId": 100, "name": "Test", "rpc": []}]
        cache_file.write_text(json.dumps(cache_data))

        with patch("aea_ledger_ethereum.chainlist._CACHE_PATH", cache_file):
            with patch("aea_ledger_ethereum.chainlist.urllib.request.urlopen") as m:
                cl = ChainlistRPC()
                cl.fetch_data()
                m.assert_not_called()
    ...
    def test_fetches_when_cache_expired(
        ...
        """Expired cache triggers a fresh network fetch."""
        cache_file = tmp_path / "rpcs.json"
        cache_dir = tmp_path
        fresh_data = [{"chainId": 100, "name": "Fresh", "rpc": []}]

        mock_resp = MagicMock()
        mock_resp.read.return_value = json.dumps(fresh_data).encode()
        mock_resp.__enter__ = MagicMock(return_value=mock_resp)
        mock_resp.__exit__ = MagicMock(return_value=False)

        with patch("aea_ledger_ethereum.chainlist._CACHE_PATH", cache_file), patch(
            "aea_ledger_ethereum.chainlist._CACHE_DIR", cache_dir
        ), patch(
            "aea_ledger_ethereum.chainlist.urllib.request.urlopen",
    ...
    def test_falls_back_to_expired_cache(
        self, tmp_path: "pytest.TempPathFactory"
    ) -> None:
        """Network failure falls back to expired cache."""
        cache_file = tmp_path / "rpcs.json"
        old_data = [{"chainId": 100, "name": "Old", "rpc": []}]
        cache_file.write_text(json.dumps(old_data))

        with patch("aea_ledger_ethereum.chainlist._CACHE_PATH", cache_file), patch(
            "aea_ledger_ethereum.chainlist.CACHE_TTL", 0
        ), patch(
            "aea_ledger_ethereum.chainlist.urllib.request.urlopen",
            ...
            cl = ChainlistRPC()
            cl.fetch_data()
            assert cl._data == old_data


# ---------------------------------------------------------------------------
# enrich_rpc_urls (main entry point)
# ---------------------------------------------------------------------------
...
class TestEnrichRpcUrls:
    ...
    @patch("aea_ledger_ethereum.chainlist.ChainlistRPC")
    def test_no_extra_rpcs_returns_original(self, mock_cl_cls: MagicMock) -> None:
        """No extra RPCs found returns original URLs."""
        mock_cl = mock_cl_cls.return_value
        mock_cl.get_validated_rpcs.return_value = []
