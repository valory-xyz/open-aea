...
"""Smoke tests for the aea-ci CLI.
...
import pytest
from aea_ci_helpers.cli import cli
...
def test_cli_imports() -> None:
    ...
    import aea_ci_helpers.cli as mod  # noqa: F401  # pylint: disable=import-outside-toplevel


def test_cli_help_runs() -> None:
    """``aea-ci --help`` exits 0 and lists the command group."""
    runner = CliRunner()
    result = runner.invoke(cli, ["--help"])
    assert result.exit_code == 0, result.output
    assert "AEA CI helper utilities" in result.output
...
def test_each_command_help_runs(command: str) -> None:
    """``aea-ci <cmd> --help`` exits 0 for every registered command.
