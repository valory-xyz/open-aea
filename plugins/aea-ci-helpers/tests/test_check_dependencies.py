...
import pytest
from aea_ci_helpers.check_dependencies import (
    PyProjectToml,
...
def test_load_returns_none_when_no_poetry_table(tmp_path: Path) -> None:
    ...
    assert PyProjectToml.load(pyproject) is None
...
def test_dump_does_not_hoist_group_deps_to_main(tmp_path: Path) -> None:
    ...
    # also covers the `file.write_text` write step in dump().
...
def test_dump_preserves_trailing_comment_on_rewrite(tmp_path: Path) -> None:
    ...
    config.dump()
    after = pyproject.read_text()
    # Version is normalized (^2.0.0 -> ==2.0.0) and the comment survives.
...
def test_dump_no_space_form_yields_no_duplicate(tmp_path: Path) -> None:
    ...
    # Result must still be valid TOML (no duplicate-key error).
    with pyproject.open("rb") as fp:
        reloaded = tomllib.load(fp)
    assert "flask" in reloaded["tool"]["poetry"]["dependencies"]
...
def test_update_warns_on_dict_form_main_dep(
    ...
    # ...but dump() leaves the dict-form line on disk untouched.
    config.dump()
...
def test_update_warns_on_string_form_group_dep(
    ...
    # In-memory mutation happened, but the group line stays verbatim.
...
def test_dump_handles_header_with_trailing_comment(tmp_path: Path) -> None:
    ...
    config.dump()
    after = pyproject.read_text()
    # The header section was recognized, so web3 got normalized.
    assert "# main runtime" in after  # header comment preserved
    assert '"^7.0.0"' not in after  # ^ normalized to ==
...
def test_load_returns_none_on_non_dict_dependencies(
    ...
    pyproject.write_text(content)
    with caplog.at_level(logging.WARNING):
        result = PyProjectToml.load(pyproject)
    assert result is None
    assert any(
        "is not a table" in r.getMessage() for r in caplog.records
    ), "expected a warning that [tool.poetry.dependencies] is not a table"
...
def test_dump_appends_new_dep_when_deps_is_last_section(tmp_path: Path) -> None:
    ...
    with pyproject.open("rb") as fp:
        reloaded = tomllib.load(fp)
    assert "newdep" in reloaded["tool"]["poetry"]["dependencies"]
...
def test_check_returns_ok_for_matching_group_dep(tmp_path: Path) -> None:
    ...
    config = PyProjectToml.load(_write_pyproject(tmp_path))
    assert config is not None
    # `Flask` is declared only in [tool.poetry.group.dev.dependencies].
...
def test_dump_handles_subtable_form_dep(tmp_path: Path) -> None:
    ...
    assert "docker" in config.dependencies
    config.dump()
    # Must re-parse (no duplicate `docker` key) and keep the sub-table.
    with pyproject.open("rb") as fp:
        reloaded = tomllib.load(fp)
...
def test_update_no_warn_for_string_main_dep_also_in_group(
    ...
    # And dump() does rewrite the main line.
    config.dump()
