...
def test_correct_echo_string():
    ...
    echo_blocks = [b for b in bash_code_blocks if b.startswith("echo '")]
    assert len(echo_blocks) > 0, "No echo block found in quickstart.md"
    default_protocol_spec_id = echo_blocks[0].split(",")[2]
    assert (
