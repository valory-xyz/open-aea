...
from aea.cli.check_packages import find_all_configuration_files, get_public_id_from_yaml, find_public_id_assignments, \
    find_public_id_from_str, find_public_id_components
...
class _TestPublicIdParameters:
    ...
    mocked_values: Dict[str, Union[List, str]]
    exit_code: int
    message: str
...
class TestCheckPackagesCommand(BaseAEATestCase):
    ...
    def test_check_public_id_failure_wrong_public_id(
        ...
        patches = [mock.patch(f"aea.cli.check_packages.{method}", return_value=mocked_value) for method, mocked_value in test_param.mocked_values.items()]
        patches.extend([_find_all_configuration_files_patch(
            [self.test_connection_config]
        ), check_dependencies_patch])
